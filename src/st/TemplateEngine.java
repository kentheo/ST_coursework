...
public class TemplateEngine {
    ...
    public String evaluate(String templateString, EntryMap entryMap, String matchingMode){
        ...
        if (matchingMode.equals(MM_OPTIMIZATION)) {
            Result keep_result = instantiate(templateString, sortedTemplates, entryMap.getEntries(), MM_KEEP);
            int keep_templates_replaced = keep_result.getTemplatesReplaced();

            Result delete_result = instantiate(templateString, sortedTemplates, entryMap.getEntries(), MM_DELETE);
            int delete_templates_replaced = delete_result.getTemplatesReplaced();

            if (keep_templates_replaced >= delete_templates_replaced) return keep_result.getInstancedString();
            else return delete_result.getInstancedString();
        }
